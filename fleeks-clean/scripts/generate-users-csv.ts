// Generate CSV for Supabase Dashboard import
const emails = [
  'epic.3jb@gmail.com',
...
const csvHeader = 'email,full_name,membership_type,membership_expires_at'
const csvRows = emails.map(email => {
  const fullName = email.split('@')[0]
  const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString()
  return `${email},${fullName},premium,${expiresAt}`
})

const csvContent = [csvHeader, ...csvRows].join('\n')

// Save to file
const outputPath = path.resolve(__dirname, '../fleeks-users-import.csv')
...
console.log('2. Navigate to Table Editor → fleeks_profiles')
console.log('3. Click "Insert rows" → "Import data from CSV"')
console.log('4. Upload the generated CSV file')
console.log('\nNote: Users will need to use "Forgot Password" feature to set their passwords.')